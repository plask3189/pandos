...
*           To allocate and deallocate pcbs, a pcbFreeList is kept. pcbFree List is a singly linked stack that holds free/unused pcbs. The head is pointed to by pcbFree_h.
*
*       2) Process Queue Maintenance:
*          Performs manipulations on the process queues (the ones in use currently). These process queues are double, circularly linked lists with a tail pointer tp
*
...
void initPcbs() {
  pcbFree_h = NULL;
	...
	for (i = 0; i < MAXPROC; i++){
		  /* The & means get the address of that element of the array. We need to get an address because freePcb() takes a pointer. So we create a pointer called addressOfPcbArrayElement. Set it to be an element's address in the pcbArray. */
...
/* Return a pointer to the first pcb from the process queue whose tail is pointed to by tp. Do not remove this pcbfrom the process queue. Return NULL if the process queue is empty. */
