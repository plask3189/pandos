...
void freePcb(pcb_t *p){
	...
	p -> p_next = pcbFree_h;
	/* Set the address of the new node to the address that the head holds. Now the head points to the new node that is on top  
	* of the stack holding the free pcbs. */
	pcbFree_h = p;
}


/* Return NULL if the pcbFree list is empty. Otherwise, remove an element from the pcbFree list, 
* provide initial values for ALL of the pcb's ﬁelds (i.e. NULL and/or 0) and then return a pointer to the removed element. 
* pcbs get reused, so it is important that no previous value persist in a pcb's when it gets reallocated. */
pcb_t *allocPcb(){
