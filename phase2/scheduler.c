 ...
 * Our goal is to have some process running at all times to maximize CPU utilization. Whenever the 
 * CPU becomes idle (like after an IO when waiting for response), the OS selects one of the pcbs in 
...
/* Start the scheduler! Round-Robin method is implemented and controls the schedule of each 
* process that needs to be executed.
* A possible situation where the scheduler() was called: a process just sent out an IO request 
* so now it's waiting. We want to put this process on pause so we can run another process while we're waiting. */

void scheduler(){
    cpu_t howManyProcessorCyclesElapsed; /* Initialize variable to track proc cycles */
    if(currentProc != NULL){ /* There is a process currently running. We want to check how long it has been 
                                executing on the CPU/ */
        STCK(howManyProcessorCyclesElapsed); /* This is an assignment statement!! We populate howManyProcessorCyclesElapsed 
                                                with the TOD clock/time scale (We need this to check how long this currently 
                                                running process has been executing). */
        currentProc->p_time = currentProc->p_time + (howManyProcessorCyclesElapsed - startTOD); /* update how long this executing 
                                                                                                process has been hogging the CPU. */
